...
app.post("/api/verify-id", upload.single("idImage"), async (req, res) => {
  try {
    ...
    await appendSheetValues(process.env.ID_VERIFICATION_SHEET_ID, "A:C", [[user.email.toLowerCase(), uploadResult.secure_url, new Date().toISOString()]]);
    res.json({ success: true, message: "ID uploaded" });
  } catch (err) { console.error(err); res.status(500).json({ success: false, message: "ID verification failed" }); }
});
