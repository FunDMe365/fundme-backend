...
app.post("/api/verify-id", upload.single("idFile"), async (req, res) => {
  ...
  if (!email || !idStatus) {
    return res.status(400).json({ success: false, message: "Missing email or idStatus" });
  }
  ...
  try {
    // Save verification info to Google Sheet
    ...
    res.json({ success: true, message: "ID verification submitted", file: uploadedFilePath });
  } catch (err) {
...
// ===== The rest of your server code (auth, campaigns, checkout, etc.) remains unchanged =====
